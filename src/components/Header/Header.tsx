...
const Header: React.FC = () => {
  ...
  return (
    <header className="w-full bg-white p-4 flex justify-end items-center fixed top-0 z-50">
      <nav>
        <ul className="flex space-x-2">
          ...
          <li className="relative z-1000">
            <button
              ...
              className="terty-color flex items-center space-x-2 px-4 py-2 rounded-lg cursor-pointer relative z-1000"
            >
              <FaUserCog className="size-5 text-white" />
              {isWideScreen && <span className="text-white">User Settings</span>}
            </button>
            {isDropdownOpen && (
              <ul className="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-lg z-1000">
                <li>
                  <NavLink
